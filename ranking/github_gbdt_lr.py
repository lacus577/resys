...
def gbdt_lr_predict_v1(data, category_feature, continuous_feature):  # 0.43616
    # 离散特征one-hot编码
    # print('开始one-hot...')
    # for col in category_feature:
    #     onehot_feats = pd.get_dummies(data[col], prefix=col)
    ...
    for col in category_feature:
        ...
        # onehot_feats = pd.DataFrame(le.transform(data[col]), columns=[col])
        onehot_feats =le.transform(data[col].astype(str))
        data.loc[:, col] = onehot_feats
        # data.drop([col], axis=1, inplace=True)
        # data = pd.concat([data, onehot_feats], axis=1)
    print('one-hot结束')
    ...
    # res = pd.read_csv('data/test.csv')
    # submission = pd.DataFrame({'Id': res['Id'], 'Label': y_pred})
    # submission.to_csv('submission/submission_gbdt+lr_trlogloss_%s_vallogloss_%s.csv' % (train_logloss, val_logloss),
    #                   index=False)
    print('结束')
